...
public class ViewToPresenterMapWriter {
    static void write(Filer filer, Map<KnitPresenterMirror, KnitViewMirror> map) {
        ...
        for(KnitPresenterMirror mirror: map.keySet()){
            ...
            createPresenterForViewMethodBuilder.addStatement("return new $L.$L$L(new $L(),modelManager)",element.getQualifiedName(),mirror.enclosingClass.getSimpleName(),KnitFileStrings.KNIT_PRESENTER_POSTFIX,mirror.enclosingClass.getQualifiedName());
